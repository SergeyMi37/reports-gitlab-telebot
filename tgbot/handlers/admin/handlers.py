...
GITLAB_LABELS = os.getenv('GITLAB_LABELS', 'Табель')
      ...
      if answer.get('data').get('issuable') is not None:
        ...
        for item in answer.get('data').get('issuable').get('timelogs').get('nodes'):
          ...
          if ggggmmdd==curday:
            userfio=''
            if mode=="name":
                userfio=f'<b>{answer_item["name"]}</b> {ggggmmdd}{BR}'
            summ += f"{userfio} {item.get('summary')}{BR+BR}"
...
def get_daily(label: str = "Табель", curday: str='', mode: str='name'):
    ...
    print('---',errno, answer)
    summ=f"{label}{BR}<b>Выполненные мероприятия за {curday}</b>{BR+BR}"
    sum=summ
    if errno == "code.CODE_GITLAB_GET_ISSUE_OK":
        for item in answer:
